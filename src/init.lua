...
local Janitor = require(script.Parent:FindFirstChild("Janitor"))
local Log = require(script.Parent:FindFirstChild("Log"))
...
local function CallFunction(Function, ...)
	return Function(...)
end
...
do
	...
	ScriptConnection.ClassName = "MadScriptConnection"
	...
	ScriptSignal.ClassName = "MadScriptSignal"
	...
	Madwork = {
		...
		ConnectToOnClose = function(task, run_in_studio_mode)
			if not RunService:IsStudio() or run_in_studio_mode == true then
...
ProfileService.ServiceLocked = false :: boolean -- Set to true once the server is shutting down
ProfileService.ProfileResult = Madwork.NewScriptSignal()
ProfileService.FireProfileResult = true :: boolean -- Set to false to disable ProfileResult signal

ProfileService.IssueSignal = Madwork.NewScriptSignal() -- (error_message, profile_store_name, profile_key) -- Fired when a DataStore API call throws an error
ProfileService.CorruptionSignal = Madwork.NewScriptSignal() -- (profile_store_name, profile_key) -- Fired when DataStore key returns a value that has
...
ProfileService.CriticalState = false -- Set to true while DataStore service is throwing too many errors
ProfileService.CriticalStateSignal = Madwork.NewScriptSignal() -- (is_critical_state) -- Fired when CriticalState is set to true
-- (You may alert players with this, or set up analytics)

ProfileService.ServiceIssueCount = 0

ProfileService._active_profile_stores = {} -- {profile_store, ...}

ProfileService._auto_save_list = {} -- {profile, ...} -- loaded profile table which will be circularly auto-saved

ProfileService._issue_queue = {} -- [table] {issue_time, ...}
ProfileService._critical_state_start = 0 -- [number] 0 = no critical state / os.clock() = critical state start

-- Debug:
ProfileService._mock_data_store = {}
ProfileService._user_mock_data_store = {}

ProfileService._use_mock_data_store = false

local ProfileResult = ProfileService.ProfileResult

local function CheckIfProfiled(Function)
	return if type(Function) == "table" then not not Function.IsProfiled else false
end

local function ProfileFunction(FunctionName: string, Function, ...)
	if CheckIfProfiled(Function) then
		return Function(...)
	end

	local StartTime = os.clock()
	local Values = table.pack(Function(...))
	local FinishTime = os.clock() - StartTime

	if ProfileService.FireProfileResult then
		local Success, Exception = pcall(function()
			ProfileResult:Fire(FunctionName, FinishTime)
		end)

		if not Success then
			Warn(string.format("Failed to fire ProfileResult signal - %s", Exception))
		end
	end
...
local function ProfileFunctionFenv(FunctionName: string, Function)
	if CheckIfProfiled(Function) then
		return Function
	end
	...
	return setmetatable({IsProfiled = true}, Metatable)
end
...
local function IdentifyProfile(store_name, store_scope, key)
	return string.format(
		"[Store:%q;%*Key:%q]",
		store_name,
		store_scope ~= nil and string.format("Scope:%q;", store_scope) or "",
		key
	)
end

local CustomWriteQueueCleanup = ProfileFunctionFenv("CustomWriteQueueCleanup", function(store, key)
...
local CustomWriteQueueMarkForCleanup = ProfileFunctionFenv("CustomWriteQueueMarkForCleanup", function(store, key)
	if CustomWriteQueue[store] ~= nil then
...
local CustomWriteQueueAsync = ProfileFunctionFenv(
	"CustomWriteQueueAsync",
	function(callback, store, key) --> ... -- Passed return from callback
		if CustomWriteQueue[store] == nil then
...
local IsCustomWriteQueueEmptyFor = ProfileFunctionFenv(
	"IsCustomWriteQueueEmptyFor",
	function(store, key) --> is_empty [bool]
...
local WaitForLiveAccessCheck = ProfileFunctionFenv("WaitForLiveAccessCheck", function()
	-- This function was created to prevent the ProfileService module yielding execution when required
	while IsLiveCheckActive == true do
		task.wait()
	end
end)

local WaitForPendingProfileStore = ProfileFunctionFenv("WaitForPendingProfileStore", function(profile_store)
...
local function NewMockDataStoreKeyInfo(params)
	...
	return {
		...
		Version = StringRep("0", 16)
			.. "."
			.. StringRep("0", 10 - #version_id_string)
			.. version_id_string
			.. "."
			.. StringRep("0", 16)
			.. "."
...
--]]
local StandardProfileUpdateAsyncDataStore = ProfileFunctionFenv(
	"StandardProfileUpdateAsyncDataStore",
	function(profile_store, profile_key, update_settings, is_user_mock, is_get_call, version) --> loaded_data, key_info
		local loaded_data, key_info
		local success, error_message = pcall(function()
			local function transform_function(latest_data)
			...
			elseif UseMockDataStore == true then -- Used when API access is disabled
				...
				loaded_data, key_info = ProfileFunction(
					"CustomWriteQueueAsync",
					CustomWriteQueueAsync,
					function() -- Callback
						if is_get_call == true then
							...
							if version ~= nil then
								local success2, error_message2 = pcall(function()
									get_data, get_key_info = ProfileFunction("GetVersionAsync", function()
										return profile_store._global_data_store:GetVersionAsync(profile_key, version)
									end)
								end)
								...
								get_data, get_key_info = ProfileFunction("GetAsync", function()
									return profile_store._global_data_store:GetAsync(profile_key)
								end)
							end

							get_data = ProfileFunction("transform_function", transform_function, get_data)
							return get_data, get_key_info
						else
							return ProfileFunction("UpdateAsync", function()
...
local RemoveProfileFromAutoSave = ProfileFunctionFenv("RemoveProfileFromAutoSave", function(profile)
...
local AddProfileToAutoSave = ProfileFunctionFenv(
	"AddProfileToAutoSave",
	function(profile) -- Notice: Makes sure this profile isn't auto-saved too soon
...
local ReleaseProfileInternally = ProfileFunctionFenv("ReleaseProfileInternally", function(profile)
...
local CheckForNewGlobalUpdates = ProfileFunctionFenv(
	"CheckForNewGlobalUpdates",
	function(profile, old_global_updates_data, new_global_updates_data)
		...
		for _, new_global_update in new_global_updates_data[2] do -- ipairs
			-- Find old global update with the same update_id:
			local old_global_update
			for _, global_update in old_global_updates_data[2] do -- ipairs
			...
			if is_new == true then
				...
				if new_global_update[3] == false then
					...
					for _, update_id in pending_update_lock do -- ipairs
					...
					if is_pending_lock == false then
						...
						global_updates_object._new_active_update_listeners:Fire(
							new_global_update[1],
							new_global_update[4]
						)
					end
				...
				if new_global_update[3] == true then
					...
					for _, update_id in pending_update_clear do -- ipairs
						if new_global_update[1] == update_id then
...
local SaveProfileAsync = ProfileFunctionFenv("SaveProfileAsync", function(profile, release_from_session, is_overwriting)
				...
				EditProfile = function(latest_data)
					...
					if session_owns_profile == true then -- We may only edit the profile if this session has ownership of the profile
						if is_overwriting ~= true then
							...
							for _, latest_global_update in latest_global_updates_list do -- ipairs
								for _, lock_id in pending_update_lock do -- ipairs
									if latest_global_update[1] == lock_id then
										latest_global_update[3] = true
										break
									end
								end
							end
							...
							for _, clear_id in pending_update_clear do -- ipairs
								for i, latest_global_update in latest_global_updates_list do -- ipairs
									if latest_global_update[1] == clear_id and latest_global_update[3] == true then
	...
	for _, global_update in self._updates_latest[2] do -- ipairs
		if global_update[3] == true then
			...
			if self._pending_update_clear ~= nil then
				for _, update_id in self._pending_update_clear do -- ipairs
					if global_update[1] == update_id then
...
function GlobalUpdates:LockActiveUpdate(update_id)
	...
	if global_update_exists ~= nil then
		local is_pending_lock = false
		for _, lock_update_id in self._pending_update_lock do -- ipairs
...
function GlobalUpdates:ClearLockedUpdate(update_id)
	...
	if global_update_exists ~= nil then
		local is_pending_clear = false
		for _, clear_update_id in self._pending_update_clear do -- ipairs
...
function GlobalUpdates:ChangeActiveUpdate(update_id, update_data)
	...
	for _, global_update in updates_latest[2] do -- ipairs
...
function Profile:StoreOnValueChange(Name: string, ValueBase)
	...
	return self._janitor:Add(ValueBase.Changed:Connect(function(NewValue)
...
function Profile:StoreOnAttributeChange(Name: string, Object: Instance)
	...
	self._used_keys[Name] = true
	return self._janitor:Add(Object:GetAttributeChangedSignal(Name):Connect(function()
		self.Data[Name] = Object:GetAttribute(Name)
	end), "Disconnect")
end
...
function Profile:StoreToTableOnAttributeChange(Name: string, TableName: string, Object: Instance)
	...
	self._used_keys[NewName] = true
	return self._janitor:Add(Object:GetAttributeChangedSignal(Name):Connect(function()
		DataTable[Name] = Object:GetAttribute(Name)
	end), "Disconnect")
end

function Profile:StoreToPathOnValueChange(Path: string | {string}, ValueBase)
	local PathArray = if type(Path) == "string" then string.split(Path, ".") else Path
	local NewName = table.concat(PathArray, ".")
...
function Profile:StoreToPathOnAttributeChange(Path: string | {string}, Object: Instance)
	local PathArray = if type(Path) == "string" then string.split(Path, ".") else Path
	local NewName = table.concat(PathArray, ".")
	local Name = table.remove(PathArray)
...
function Profile:Set(Key: string, Value: any)
	self.Data[Key] = Value
	self.DataUpdated:Fire(Key, Value)
end

function Profile:RawSet(Key: string, Value: any)
	self.Data[Key] = Value
end

function Profile:SetToPath(Path: string | {string}, Value: any)
	local PathArray = if type(Path) == "string" then string.split(Path, ".") else Path
	local NewName = table.concat(PathArray, ".")
	local Key = table.remove(PathArray)
	...
	DataTable[Key] = Value
	self.DataUpdated:Fire(NewName, Value)
end

function Profile:RawSetToPath(Path: string | {string}, Value: any)
	local PathArray = if type(Path) == "string" then string.split(Path, ".") else Path
	local NewName = table.concat(PathArray, ".")
	local Key = table.remove(PathArray)
...
function Profile:ListenToRelease(listener) --> [ScriptConnection] (place_id / nil, game_job_id / nil)
	...
	if self:IsActive() == false then
		...
		return self._janitor:Add(self._release_listeners:Connect(listener), "Disconnect")
	end
end
...
Profile.Save = ProfileFunctionFenv("Profile.Save", function(self)
	...
	if IsCustomWriteQueueEmptyFor(self._profile_store._profile_store_lookup, self._profile_key) == true then
		...
		task.spawn(CallFunction, SaveProfileAsync, self)
	end
end)

function Profile:SaveAsync()
	if self._view_mode == true then
		return Promise.reject("Can't save Profile in view mode - Should you be calling :OverwriteAsync() instead?")
	end
	...
	return Promise.new(function(Resolve, Reject)
		if IsCustomWriteQueueEmptyFor(self._profile_store._profile_store_lookup, self._profile_key) == true then
			...
			AddProfileToAutoSave(self)
			-- Call save function in a new thread:
			local Success, Value = SaveProfileAsync(self);
			(if Success then Resolve else Reject)(Value)
...
function Profile:Release()
	...
	if self:IsActive() == true then
		task.spawn(CallFunction, SaveProfileAsync, self, true) -- Call save function in a new thread with release_from_session = true
	end
end
...
function Profile:ListenToHopReady(listener) --> [ScriptConnection] ()
	...
	if self._hop_ready == true then
		...
		return self._janitor:Add(self._hop_ready_listeners:Connect(listener), "Disconnect")
	end
end
...
Profile.OverwriteAsync = Promise.promisify(Profile.Overwrite)
...
function Profile:__tostring()
	return string.format("Profile<%*>", self:Identify())
end
...
ProfileVersionQuery.ClassName = "ProfileVersionQuery"
...
function ProfileVersionQuery:Next(_is_stacking) --> [Profile] or nil
	...
	local function query_job()
		...
		self._query_index += 1
		profile = self._profile_store:ViewProfile(self._profile_key, next_item.Version)
		is_finished = true
	end
...
ProfileVersionQuery.NextAsync = Promise.promisify(ProfileVersionQuery.Next)

function ProfileVersionQuery:__tostring()
...
ProfileStore.ClassName = "ProfileStore"
ProfileStore.__index = ProfileStore

type ReleaseHandlerEnum = "ForceLoad" | "Steal"
type AllReleaseHandlerEnum = ReleaseHandlerEnum | "Repeat" | "Cancel"

ProfileStore.LoadProfile = ProfileFunctionFenv(
	"ProfileStore.LoadProfile",
	function(
		self,
		profile_key: string,
		notReleasedHandler: nil | ReleaseHandlerEnum | (PlaceId: number, GameJobId: number) -> AllReleaseHandlerEnum,
		_use_mock
	) --> [Profile / nil] not_released_handler(place_id, game_job_id)
		...
		while ProfileService.ServiceLocked == false do
			...
			if profile_load_job ~= nil then
				...
				if profile_load_job[1] == load_id then -- Load job hasn't been double-yoinked
					loaded_data, key_info = table.unpack(profile_load_job[2], 1, 2)
							...
							elseif handler_result == "Steal" then
								...
								Error(
									"Invalid return from not_released_handler (\""
										...
										.. "\n"
										.. IdentifyProfile(
											self._profile_store_name,
											self._profile_store_scope,
											profile_key
										)
										.. " Traceback:\n"
										.. debug.traceback()
...
function ProfileStore:__tostring()
	return "ProfileStore"
end

-- New ProfileStore:

export type ProfileStoreIndex = string | {
	Name: string,
	Scope: string?,
}

function ProfileService.GetProfileStore<T>(profile_store_index: ProfileStoreIndex, profile_template: T & {[any]: any}) --> [ProfileStore]
	...
	self = {
		Mock = {
			LoadProfile = function(_, profile_key, not_released_handler)
				return self:LoadProfile(profile_key, not_released_handler, UseMockTag)
			end;

			GlobalUpdateProfile = function(_, profile_key, update_handler)
				return self:GlobalUpdateProfile(profile_key, update_handler, UseMockTag)
			end;

			ViewProfile = function(_, profile_key, version)
				return self:ViewProfile(profile_key, version, UseMockTag)
			end;

			FindProfileVersion = function(_, profile_key, sort_direction, min_date, max_date)
				return self:FindProfileVersion(profile_key, sort_direction, min_date, max_date, UseMockTag)
			end;

			WipeProfile = function(_, profile_key)
				return self:WipeProfile(profile_key, UseMockTag)
			end;

			LoadProfileAsync = function(_, profile_key, not_released_handler)
				return self:LoadProfileAsync(profile_key, not_released_handler, UseMockTag)
			end;

			GlobalUpdateProfileAsync = function(_, profile_key, update_handler)
				return self:GlobalUpdateProfileAsync(profile_key, update_handler, UseMockTag)
			end;

			ViewProfileAsync = function(_, profile_key, version)
				return self:ViewProfileAsync(profile_key, version, UseMockTag)
			end;

			FindProfileVersionAsync = function(_, profile_key, sort_direction, min_date, max_date)
				return self:FindProfileVersionAsync(profile_key, sort_direction, min_date, max_date, UseMockTag)
			end;

			WipeProfileAsync = function(_, profile_key)
				return self:WipeProfileAsync(profile_key, UseMockTag)
			end;
		};
	...
	if IsLiveCheckActive == true then
		...
		task.spawn(function()
			...
			if UseMockDataStore == false then
				self._global_data_store = DataStoreService:GetDataStore(profile_store_name, profile_store_scope)
			end
...
RunService.Heartbeat:Connect(function()
	...
	if auto_save_list_length > 0 then
		...
		while os_clock - LastAutoSave > auto_save_index_speed do
			...
			-- Perform save call:
			if profile ~= nil then
				task.spawn(CallFunction, SaveProfileAsync, profile) -- Auto save profile in new thread
			end
		end
	end
